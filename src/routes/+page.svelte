<script lang="ts">
 ...
 import { Activity, Scale, Percent, Heart, Trash2, Trash } from "lucide-svelte";

 let latest = $derived(formaStore.latest);
 let stats = $derived(formaStore.stats);
  ...
  <div class="dashboard-grid">
   ...
   <div class="summary-cards">
    ...
    <MetricCard
     ...
     unit="kg"
     delta={latest!.muscleMass -
      (formaStore.previous?.muscleMass ?? latest!.muscleMass)}
    />
    <MetricCard
     label="BMI"
     value={latest!.bmi.toFixed(1)}
     unit=""
     delta={latest!.bmi - (formaStore.previous?.bmi ?? latest!.bmi)}
     inverse={true}
    />
   </div>
