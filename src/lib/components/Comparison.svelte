<script lang="ts">
 ...
 let startLabel = $derived(
  history.find((h) => h.id === startId)?.label ?? "Start",
 );
    ...
    <div class="selector-container">
     ...
     {#if showStartSelector}
      ...
      <button
       ...
       onclick={() => (showStartSelector = false)}
    ...
    {#if config}
     ...
     <div class="metric-item" class:active={activeSelectorIndex === i}>
      <div class="metric-header">
       ...
       <div class="selector-wrapper">
        <button
         class="btn-selector"
         onclick={() =>
          (activeSelectorIndex = activeSelectorIndex === i ? null : i)}
      ...
      <div class="comparison-row">
       <div class="val-box">
        <span class="val"
         >{config.format(startRecord[config.key] as number)}</span
        >
       ...
       <div class="val-box">
        <span class="valHighlight"
         >{config.format(endRecord[config.key] as number)}</span
        >
        <span class="u">{config.unit}</span>
       </div>
      </div>
      ...
      <div
       class="delta-box"
       class:good={isGood && !isNeutral}
       class:bad={!isGood && !isNeutral}
      ...
      >
       ...
       {:else}
        ...
        {/if}
        <span
         >{Math.abs(diff.val).toFixed(1)}{config.unit} ({Math.abs(
          diff.percent,
         ).toFixed(1)}%)</span
        >
       {/if}
